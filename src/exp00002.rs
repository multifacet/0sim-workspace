...
pub fn run<A: std::net::ToSocketAddrs + std::fmt::Display>(
    ...
    let (mut ushell, vshell) =
        connect_and_setup_host_and_vagrant(dry_run, &cloudlab, username, vm_size)?;
...
pub fn run_setup_only<A: std::net::ToSocketAddrs + std::fmt::Display>(
    ...
    // Connect
    let _ = connect_and_setup_host_and_vagrant(dry_run, &cloudlab, username, vm_size)?;
...
fn connect_and_setup_host_and_vagrant<A: std::net::ToSocketAddrs + std::fmt::Display>(
    ...
    let vshell = start_vagrant(&ushell, &cloudlab, vm_size)?;
